(function() {
    ...
    module.config(['$stateProvider', '$urlRouterProvider', '$ionicConfigProvider', function($stateProvider, $urlRouterProvider, $ionicConfigProvider) {
        ...
        // Form - detaildoc
        $stateProvider.state('app.detaildoc', {
            url: '/detaildoc/:id',
            // This allows the parameter to be optional without a trailing slash
            params: {
                id: {squash:true, value:null}
            ...
            views: {
                'menuContent': {
                    template:
                       '<ion-view view-title="{{getTitle()}}">'
                      +'<ion-content>'
                      +'<div ng-include src="\'templates/detaildoc.html\'"></div>'
                      +'</ion-content>'
                      +'</ion-view>'
                    ,
        ...
        // Form - editdoc
        $stateProvider.state('app.editdoc', {
            url: '/editdoc/:id',
            // This allows the parameter to be optional without a trailing slash
            params: {
                id: {squash:true, value:null}
            ...
            views: {
                'menuContent': {
                    template:
                      ...
                      +'</ion-nav-buttons>'
                      +'<ion-content>'
                      +'<div ng-include src="\'templates/editdoc.html\'"></div>'
                      +'</ion-content>'
                      +'</ion-view>'
                    ,
    ...
    module.controller('detaildocCtrl', ['session','notifyDataChange','$controller','$scope', '$state','$stateParams','$ionicHistory', function(session,notifyDataChange,$controller,$scope,$state,$stateParams,$ionicHistory) {
    ...
    module.controller('editdocCtrl', ['session','notifyDataChange','$controller','$scope', '$state','$stateParams','$ionicHistory', function(session,notifyDataChange,$controller,$scope,$state,$stateParams,$ionicHistory) {
    	// Inherit from a base View controller
    	$controller('AbstractFormDocCtrl', {$scope: $scope});
    ...
    module.controller('byauthorCtrl1', ['$controller','$scope','$state','$jstore','session', function($controller,$scope,$state,$jstore,session) {
    	...
    	$scope.onEntryClick = function(entry) {
    		...
    		if(!$scope.isDualPane()) {
    			$state.go('app.editdoc',{id:entry.unid},{reload:true});
    		}
    	}	
    }]);
